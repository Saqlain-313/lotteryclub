...
const MatkaMarkets = () => {
  ...
  const getMarketBadge = (marketName) => {
    ...
    return found ? badges[found] : { color: "from-gray-400 to-gray-500", icon: Sparkles };
  };
  ...
  const filteredMarkets = activeMarkets?.filter((market) => {
    ...
    const matchGameType = filterGameType ? market.gameType === filterGameType : true;
    ...
    return (
      <div className="flex justify-center items-center min-h-[60vh]">
        <div className="relative">
          ...
          <p className="text-gray-500 text-sm mt-4 text-center font-medium">Loading markets...</p>
        </div>
      </div>
    );
  }
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                <div className="relative group/input">
                  <Search size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-hover/input:text-amber-500 transition" />
                  <input
                    type="text"
                ...
                <div className="relative group/select">
                  <Filter size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-hover/select:text-amber-500 transition" />
                  <select
        ...
        {filteredMarkets?.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {filteredMarkets.map((market) => {
              ...
              return (
                ...
                >
                  <div className={`absolute -inset-1 bg-gradient-to-r ${badge.color} rounded-2xl blur-xl opacity-20 group-hover/card:opacity-40 transition duration-500`}></div>
                  <div className="relative bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/50 p-5 overflow-hidden transition-all duration-300 hover:border-amber-200">
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <div className={`inline-block p-1 rounded-lg bg-gradient-to-r ${badge.color} shadow-lg`}>
                            <BadgeIcon size={10} className="text-white" />
                          </div>
                        </div>
                        <p className="text-xs text-gray-400 font-mono">ID: {market.marketId}</p>
                      </div>
                    ...
                    <div className="space-y-2.5 text-sm">
                      <div className="flex justify-between items-center">
                        <span className="text-gray-500 font-medium">Game Type</span>
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-full bg-gradient-to-r ${gameTypeGradient} text-white shadow-lg transform group-hover/card:scale-105 transition duration-300`}>
                      ...
                      <div className="flex justify-between items-center bg-gray-50/50 backdrop-blur-sm px-3 py-2 rounded-xl">
                        ...
                        <span className="font-bold text-gray-700">{market.openTime}</span>
                      </div>
                      <div className="flex justify-between items-center bg-gray-50/50 backdrop-blur-sm px-3 py-2 rounded-xl">
                        <span className="text-gray-500 flex items-center gap-1.5">
                          <Clock size={14} className="text-red-400" /> Close
                        </span>
                        <span className="font-bold text-gray-700">{market.closeTime}</span>
                      </div>
                      <div className="flex justify-between items-center bg-gradient-to-r from-amber-50/50 to-orange-50/50 backdrop-blur-sm px-3 py-2 rounded-xl border border-amber-200/50">
                        <span className="text-gray-500 flex items-center gap-1.5">
                          <TrendingUp size={14} className="text-amber-500" /> Range
                        </span>
                      ...
                      {market.resultTime && (
                        <div className="flex justify-between items-center bg-purple-50/50 backdrop-blur-sm px-3 py-2 rounded-xl border border-purple-200/50">
                          <span className="text-gray-500 flex items-center gap-1.5">
                            <Clock size={14} className="text-purple-400" /> Result
                          </span>
                          <span className="font-bold text-purple-700">{market.resultTime}</span>
                        </div>
                      )}
                    </div>
                    ...
                    {/* Place Bid Button */}
                    <button
                      onClick={() => handlePlaceBid(market._id, market.gameType)}
                    ...
                    >
                      <div className={`absolute -inset-1 bg-gradient-to-r ${badge.color} rounded-xl blur opacity-30 group-hover/btn:opacity-50 transition duration-500`}></div>
                      <div className={`relative bg-gradient-to-r ${badge.color} py-3 rounded-xl flex items-center justify-center gap-2 text-white font-bold shadow-lg shadow-amber-500/30`}>
                        <Zap size={16} />
                        Place Bid
          ...
          <div className="group relative">
            ...
            <div className="relative bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/50 p-16 text-center">
              ...
              <p className="text-gray-600 text-xl font-semibold">No active markets available</p>
              <p className="text-gray-400 text-sm mt-2">Check back later for new markets</p>
            </div>
          </div>
        )}
