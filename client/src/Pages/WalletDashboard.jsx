...
import { TrendingUp, Gift, Rocket, ArrowDownLeft, ArrowUpRight } from "lucide-react";
import {
...
export default function WalletDashboard() {
  ...
  const getWeeklyData = () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayDeposits = deposits?.filter(d =>
        d.createdAt && new Date(d.createdAt).toISOString().split('T')[0] === dateStr
      ).reduce((sum, d) => sum + (d.amount || 0), 0) || 0;

      const dayWithdrawals = withdrawals?.filter(w =>
        (w.requestedAt || w.createdAt) && 
        new Date(w.requestedAt || w.createdAt).toISOString().split('T')[0] === dateStr
  ...
  if (depositsLoading && withdrawalsLoading && deposits.length === 0 && withdrawals.length === 0) {
    return (
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
          ...
          <div className="lg:col-span-5">
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-purple-500 via-purple-600 to-indigo-700 shadow-xl p-5 sm:p-6 md:p-8 min-h-[200px] sm:min-h-[240px] md:min-h-[280px] lg:min-h-[320px] flex flex-col justify-between text-white">
              <div className="absolute -right-4 -bottom-4 opacity-20">
                <span className="text-[80px] sm:text-[100px] md:text-[120px]">💎</span>
              </div>

              <div className="relative z-10">
                <h3 className="text-lg sm:text-xl md:text-2xl font-bold">Winzox Elite Card</h3>
                <p className="text-white/80 text-xs sm:text-sm mt-1">Manage your virtual assets in 3D space</p>
              </div>
              ...
              <div className="absolute right-6 sm:right-8 bottom-6 sm:bottom-8 w-8 h-7 sm:w-10 sm:h-8 md:w-12 md:h-9 bg-gradient-to-br from-yellow-300 to-yellow-500 rounded-lg opacity-60"></div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 sm:mt-6 md:mt-8 grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
          <div className="bg-white rounded-2xl p-4 sm:p-5 md:p-6 text-center shadow-sm border border-gray-100 hover:shadow-md hover:border-purple-200 transition-all duration-300">
            <div className="w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 rounded-2xl bg-purple-100 text-purple-600 flex items-center justify-center mx-auto mb-2 sm:mb-3 md:mb-4">
              <ArrowDownLeft size={20} className="sm:w-5 sm:h-5 md:w-6 md:h-6" />
            </div>
            <p className="text-gray-500 text-[10px] sm:text-xs uppercase tracking-wider font-bold">Deposits</p>
            <h4 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold mt-1 text-gray-900">
              {currencySymbol}{totalDeposits.toLocaleString('en-IN')}
            </h4>
            <p className="text-[10px] sm:text-xs text-gray-400 mt-1">{deposits?.length || 0} transactions</p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-5 md:p-6 text-center shadow-sm border border-gray-100 hover:shadow-md hover:border-red-200 transition-all duration-300">
            ...
            <p className="text-gray-500 text-[10px] sm:text-xs uppercase tracking-wider font-bold">Withdrawals</p>
            <h4 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold mt-1 text-gray-900">
              {currencySymbol}{totalWithdrawals.toLocaleString('en-IN')}
            </h4>
            <p className="text-[10px] sm:text-xs text-gray-400 mt-1">{withdrawals?.length || 0} transactions</p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-5 md:p-6 text-center shadow-sm border border-gray-100 hover:shadow-md hover:border-green-200 transition-all duration-300">
            ...
            <p className="text-gray-500 text-[10px] sm:text-xs uppercase tracking-wider font-bold">Winnings</p>
            <h4 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold mt-1 text-gray-900">
              {currencySymbol}{totalWinnings.toLocaleString('en-IN')}
            </h4>
            <p className="text-[10px] sm:text-xs text-gray-400 mt-1">+12.5% this month</p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-5 md:p-6 text-center shadow-sm border border-gray-100 hover:shadow-md hover:border-yellow-200 transition-all duration-300">
            ...
            <p className="text-gray-500 text-[10px] sm:text-xs uppercase tracking-wider font-bold">Bonuses</p>
            <h4 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold mt-1 text-gray-900">
              {currencySymbol}{totalBonuses.toLocaleString('en-IN')}
            </h4>
            <p className="text-[10px] sm:text-xs text-gray-400 mt-1">3 active bonuses</p>
          </div>
        </div>
        ...
        <div className="mt-4 sm:mt-6 md:mt-8 bg-white rounded-2xl p-4 sm:p-6 md:p-8 shadow-sm border border-gray-100">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 md:mb-8">
            <div>
              <h3 className="text-base sm:text-lg md:text-xl lg:text-2xl font-bold text-gray-900">Weekly Balance Analytics</h3>
              <p className="text-[10px] sm:text-xs md:text-sm text-gray-500 mt-0.5 sm:mt-1">Net balance change (Deposits - Withdrawals)</p>
            </div>
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4 mt-2 sm:mt-0">
              <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2">
                ...
                <span className="text-[9px] sm:text-[10px] md:text-xs text-gray-600">Deposits</span>
              </div>
              <div className="flex items-center gap-1 sm:gap-1.5 md:gap-2">
                <span className="w-2 h-2 sm:w-2.5 sm:h-2.5 md:w-3 md:h-3 bg-red-500 rounded-full"></span>
                <span className="text-[9px] sm:text-[10px] md:text-xs text-gray-600">Withdrawals</span>
              </div>
          ...
          {hasData ? (
            <>
              <div className="h-40 sm:h-48 md:h-56 lg:h-64 flex items-end justify-between space-x-0.5 xs:space-x-1 sm:space-x-1.5 md:space-x-2 lg:space-x-3">
                {weeklyData.map((data, index) => {
                  ...
                  return (
                    <div key={index} className="flex-1 flex flex-col items-center group h-full min-w-[20px] sm:min-w-[28px] md:min-w-[32px]">
                      {/* Bar container */}
                      <div className="w-full flex items-end justify-center space-x-0.5 xs:space-x-1 sm:space-x-1 md:space-x-1.5 lg:space-x-2 h-[calc(100%-28px)] sm:h-[calc(100%-30px)] md:h-[calc(100%-32px)]">
                        ...
                        <div className="flex flex-col items-center justify-end w-3 sm:w-4 md:w-5 lg:w-6 xl:w-7 h-full">
                          <div
                            ...
                            style={{ height: `${depositHeight}%`, minHeight: '4px' }}
                          >
                            {/* Tooltip */}
                            <div className="absolute -top-8 sm:-top-9 md:-top-10 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-[8px] sm:text-[10px] md:text-xs rounded-lg px-1.5 sm:px-2 md:px-2.5 py-1 sm:py-1.5 md:py-2 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
                              Deposit: {currencySymbol}{data.deposits.toLocaleString('en-IN')}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-end w-3 sm:w-4 md:w-5 lg:w-6 xl:w-7 h-full">
                          <div
                            ...
                            style={{ height: `${withdrawalHeight}%`, minHeight: '4px' }}
                          >
                            {/* Tooltip */}
                            <div className="absolute -top-8 sm:-top-9 md:-top-10 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-[8px] sm:text-[10px] md:text-xs rounded-lg px-1.5 sm:px-2 md:px-2.5 py-1 sm:py-1.5 md:py-2 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10 pointer-events-none">
                              Withdrawal: {currencySymbol}{data.withdrawals.toLocaleString('en-IN')}
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-center mt-0.5 sm:mt-1 md:mt-1.5">
                        ...
                        <div className={`text-[6px] xs:text-[7px] sm:text-[8px] md:text-[9px] lg:text-[10px] font-bold ${data.amount >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {data.amount >= 0 ? '+' : ''}{currencySymbol}{Math.abs(data.amount).toLocaleString('en-IN')}
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className="flex flex-wrap justify-center sm:justify-between gap-1.5 sm:gap-2 md:gap-3 mt-3 sm:mt-4 md:mt-5 lg:mt-6 text-[9px] sm:text-[10px] md:text-xs text-gray-500 border-t pt-2.5 sm:pt-3 md:pt-4">
                ...
                <span className="flex items-center gap-1 sm:gap-1.5 md:gap-2 font-medium text-gray-700">
                  ...
                  Net: {currencySymbol}{weeklyData.reduce((sum, d) => sum + d.amount, 0).toLocaleString('en-IN')}
                </span>
              </div>
            </>
          ) : (
            <div className="h-40 sm:h-48 md:h-56 lg:h-64 flex items-center justify-center text-gray-400">
              <div className="text-center px-4">
                <p className="text-sm sm:text-base md:text-lg">No transaction data available</p>
                <p className="text-[10px] sm:text-xs md:text-sm mt-1">Make your first deposit or withdrawal to see analytics</p>
              </div>
            </div>
          )}
        </div>
        ...
        <footer className="mt-6 sm:mt-8 bg-white/80 backdrop-blur-xl rounded-2xl p-4 sm:p-5 border border-gray-100 shadow-sm">
          <div className="flex flex-col md:flex-row justify-between items-center text-gray-500 text-[10px] sm:text-xs md:text-sm">
            <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 md:gap-6 mb-2 md:mb-0">
              ...
              <a className="hover:text-purple-600 transition-colors" href="#">Privacy Policy</a>
              <a className="hover:text-purple-600 transition-colors" href="#">Terms of Service</a>
              <a className="hover:text-purple-600 transition-colors" href="#">Responsible Gaming</a>
            </div>
